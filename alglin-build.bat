...
@echo.
@powershell -command write-host -foreground "green" -background "black" -nonewline "YEAR=%YEAR%, BITS=%BITS%, LAPACK=%LPK%"
@echo.
...
@echo.
@powershell -command write-host -foreground "green" -background "black" -nonewline "Select Lapack Type: %LPK%"
@echo.
...
@IF "%LPK%" == "MKL" (
  ...
  @echo.
  @powershell -command write-host -foreground "red" -background "DarkGray" -nonewline "Unsupported %LPK%"
  @echo.
  GOTO:eof
)
...
@echo.
@powershell -command write-host -foreground "green" -background "black" -nonewline "Select Compiler Visual Studio %YEAR% "
@echo.
...
@IF %YEAR% == 2010 (
  ...
  @echo.
  @powershell -command write-host -foreground "red" -background "DarkGray" -nonewline "Unsupported %YEAR%"
  @echo.
  GOTO:eof
)

@echo.
@powershell -command write-host -foreground "green" -background "black" -nonewline "Select Architecture %BITS%"
@echo.

@IF "%BITS%" NEQ "x86" IF "%BITS%" NEQ "x64" (
  @echo.
  @powershell -command write-host -foreground "green" -background "black" -nonewline "Unsupported ARCH %BITS%"
  @echo.
  GOTO:eof
)
...
@echo.
@powershell -command write-host -foreground "green" -background "black" -nonewline "YEAR=%YEAR%, BITS=%BITS%, LAPACK=%LPK%, COMPILE=%COMPILE%"
@echo.
...
@IF %COMPILE% == "YES" (
  ...
  @cd %VSDIR%
  @echo.
  @powershell -command write-host -foreground "yellow" -background "black" -nonewline "cmake -G \"%STR%\" -D%LPK%=1 -DYEAR=%YEAR% -DBITS=%BITS% -DCMAKE_INSTALL_PREFIX:PATH=..\lib .."
  @echo.
  @cmake -G "%STR%" -DBUILD_EXECUTABLE=1  -D%LPK%=1 -DYEAR=%YEAR% -DBITS=%BITS% -DCMAKE_INSTALL_PREFIX:PATH=..\lib ..
  @cmake --build . --config Release --target Install
  @cmake -G "%STR%" -DBUILD_EXECUTABLE=1 -D%LPK%=1 -DYEAR=%YEAR% -DBITS=%BITS% -DCMAKE_INSTALL_PREFIX:PATH=..\lib_debug ..
  @cmake --build . --config Debug --target Install
  @cd ..
) ELSE (
  @echo.
  @powershell -command write-host -foreground "yellow" -background "black" -nonewline "Alglin already compiled"
  @echo.
)

...
│
  ...
  BlockLU<t_Value>::solve_internal(
    ...
    // The level-3 BLAS path aliases input and output submatrices inside `in_out`.
    // BLAS does not guarantee correctness under overlap, so solve each RHS with
    // the already-correct single-column path.
    for ( integer k{0}; k < nrhs; ++k )
      this->solve_internal( do_permute, in_out + k * ldRhs );
  }

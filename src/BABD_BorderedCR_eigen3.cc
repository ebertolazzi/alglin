...
namespace alglin {
  ...
  BorderedCR_eigen3<t_Value>::buildT(
    ...
    case BORDERED_LU:
      ...
      break;
    case BORDERED_QR:
      info = geqrf( n_x_2, n, T, n_x_2, T+2*n_x_n, WorkQR[size_t(nth)], LworkQR );
      break;
    case BORDERED_QRP:
      { integer * P = perm_thread[size_t(nth)];
        std::fill( P, P+n, 0 );
        info = geqp3( n_x_2, n, T, n_x_2, P, T+2*n_x_n, WorkQR[size_t(nth)], LworkQR );
  ...
  BorderedCR_eigen3<t_Value>::factorize_block( integer nth ) {
    ...
    Eigen::Map<dmat_t> F_mat(Fmat[size_t(nth)],nr,nx);

...
namespace alglin {
  ...
  BorderedCR_eigen3<t_Value>::factorize() {
    if ( usedThread > 1 ) {
      ...
      for ( integer nt = 1; nt < usedThread; ++nt ) {
        ...
        TP->run( nt, &BorderedCR_eigen3<t_Value>::factorize_block, this, nt );
      }
      factorize_block(0);
      // wait thread
      TP->wait_all();
  ...
  BorderedCR_eigen3<t_Value>::solve( valueType x[] ) const {
    ...
    if ( usedThread > 1 ) {
      if ( nr > 0 ) {
        for ( integer nt = 1; nt < usedThread; ++nt ) {
          ...
          TP->run( nt, &BorderedCR_eigen3<t_Value>::forward, this, nt, x, xb_thread[nt] );
        }
    ...
    if ( usedThread > 1 ) {
      backward_reduced(x);
      for ( integer nt = 1; nt < usedThread; ++nt )
        TP->run( nt, &BorderedCR_eigen3<t_Value>::backward, this, nt, x );
      backward(0,x);
      TP->wait_all();
    } else {
      backward(0,x);
  ...
  BorderedCR_eigen3<t_Value>::solve(
    ...
    if ( usedThread > 1 ) {
      for ( integer nt = 1; nt < usedThread; ++nt )
        TP->run( nt, &BorderedCR_eigen3<t_Value>::forward_n, this, nt, nrhs, rhs, ldRhs );
    ...
    if ( usedThread > 1 ) {
      ...
      for ( integer nt = 1; nt < usedThread; ++nt )
        TP->run( nt, &BorderedCR_eigen3<t_Value>::backward_n, this, nt, nrhs, rhs, ldRhs );

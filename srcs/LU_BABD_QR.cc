  ...
  BabdQR<t_Value>::reduction() {
    ...
    while ( jump_block < nblock ) {
      ...
      for ( integer k = 0 ; k < kend ; k += kstep ) {
        ...
        Eigen::HouseholderQR<matType> & QR = QR_blk[k1] ;

        /*\
         |  / F \ = Q / R \
         |  \ E1/     \ 0 /
        \*/
        gecopy( n, n, F,  n, tmpM.data(),   nx2 ) ;
        gecopy( n, n, E1, n, tmpM.data()+n, nx2 ) ;
        QR.compute(tmpM) ;
        ...
        gecopy( n, n, E, n, tmpM.data(),   nx2 ) ;
        gezero( n, n,       tmpM.data()+n, nx2 ) ;
        tmpM1.noalias() = QR.householderQ().transpose() * tmpM ;
        gecopy( n, n, tmpM1.data(),   nx2, E1, n ) ; // memorizzo scambiate
        gecopy( n, n, tmpM1.data()+n, nx2, E,  n ) ;

        gezero( n, n,        tmpM.data(),   nx2 ) ;
        gecopy( n, n, F1, n, tmpM.data()+n, nx2 ) ;
        tmpM1.noalias() = QR.householderQ().transpose() * tmpM ;
        gecopy( n, n, tmpM1.data(),   nx2, F1, n ) ; // memorizzo scambiate
        gecopy( n, n, tmpM1.data()+n, nx2, F,  n ) ;
      }
      jump_block *= 2 ;
    }
  }
  ...
  BabdQR<t_Value>::forward_reduce( valuePointer y ) const {
    while ( jump_block < nblock ) {
      ...
      for ( integer k = 0 ; k < kend ; k += k_step ) {
        integer k1 = k+jump_block ;
        Eigen::HouseholderQR<matType> const & QR = QR_blk[k1] ;
